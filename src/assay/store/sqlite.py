...
DEFAULT_DB_PATH = Path(".assay/results.db")
    ...
    def save_result(self, result: SuiteResult) -> str:
        ...
        self._conn.execute(
            """INSERT INTO assay_run
               (id, suite_name, status, total, passed, failed, warned, errored,
                quality_score, duration_ms, started_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                run_id,
                result.suite_name,
            ...
            self._conn.execute(
                """INSERT INTO assay_check_result
                   (run_id, check_name, check_type, status, severity,
                    ...
                    row_count, failing_rows, failing_query, duration_ms)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    run_id,
                    check.check_name,
    ...
    def _store_metrics(self, suite_name: str, check: CheckResult) -> None:
        ...
        if check.check_type == "row_count":
            try:
                ...
                self._conn.execute(
                    "INSERT INTO assay_metric (suite_name, check_name, metric_name, value, recorded_at) VALUES (?, ?, ?, ?, ?)",
