    ...
    for check in checks:
        ...
        if check.check_type == "not_null":
            ...
            plan.add_metric(
                ...
                expression=f"COUNT(DISTINCT {col})",
                check_config=check,
            )
            ...
            plan.add_metric(
                ...
                expression=f"MIN({col})",
                check_config=check,
            )
            plan.add_metric(
                alias=f"range_{col}_max",
                expression=f"MAX({col})",
                check_config=check,
            )
            ...
            plan.add_metric(
                ...
                expression=f"COUNT(*) FILTER (WHERE {col} NOT IN ({placeholders}) AND {col} IS NOT NULL)",
                check_config=check,
            )
        ...
        for metric in plan.metrics:
            ...
            if check.check_type == "not_null":
                ...
                results.append(CheckResult(
                    ...
                    failing_rows_query=f"SELECT * FROM {plan.table} WHERE {col} IS NULL" if null_count > 0 else "",
                ))
                ...
                results.append(CheckResult(
                    ...
                    failing_rows_query=(
                        f"SELECT {col}, COUNT(*) as cnt FROM {plan.table} "
